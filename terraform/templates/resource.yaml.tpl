...
{{- range $spec.resources }}
  ...
  /providers/{{ .provider }}/{{ $spec.version }}{{ range .hierarchy }}/{{ . }}s/{{ printf "{%s}" . }}{{ end }}/{{ .plural | strings.Slug }}:
    get:
      tags:
        - {{ .name | strings.Title }}
      security:
...
{{ .example.list | data.ToYAML | indent 16 }}
  ...
  /providers/{{ .provider }}/{{ $spec.version }}{{ range .hierarchy }}/{{ . }}s/{{ printf "{%s}" . }}{{ end }}/{{ .plural | strings.Slug }}/{name}:
